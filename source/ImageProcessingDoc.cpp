...
BOOL CImageProcessingDoc::OnSaveDocument(LPCTSTR lpszPathName)
{
	...
	if (SaveDlg.DoModal() == IDOK) {
		...
		File.Open(SaveDlg.GetPathName(), CFile::modeCreate |
			CFile::modeWrite);
		// 파일 열기
		File.Write(m_OutputImage, m_Re_size); // 파일 쓰기
		File.Close(); // 파일 닫기
	}
...
{
	// TODO: 여기에 구현 코드 추가.
	int i, j, k, m, h, w, dis, Re_h, Re_w;
	double temp, value, dis2, rate;
	...
	if (dlg.DoModal() == IDOK) { // DoModal 대화상자의 활성화 여부
		...
		// 확대 영상을 위한 메모리 할당
		int** p_p_x;
		p_p_x = new int* [dlg.m_UpSampleRate]; // 포인터들의 배열이기 때문에 int*을 사용
		for (int i = 0; i < dlg.m_UpSampleRate; i++)  //각 포인터에는 정수 배열의 주소를 저장
		{
			p_p_x[i] = new int[dlg.m_UpSampleRate];
		}
		for (int i = 0; i < dlg.m_UpSampleRate; i++)
		{
			for (int j = 0; j < dlg.m_UpSampleRate; j++)
			{
				p_p_x[i][j] = (dlg.m_UpSampleRate-i)*(dlg.m_UpSampleRate-j);
			}
		}
		rate = dlg.m_UpSampleRate;
		for (i = 0; i < m_Re_size; i++)
			m_OutputImage[i] = 0; // 초기화

		for (i = 0; i < m_height; i++) {
			for (j = 0; j < m_width; j++) {
				h = i * dlg.m_UpSampleRate * m_Re_width;
				w = j * dlg.m_UpSampleRate;
				m_OutputImage[h + w] = m_InputImage[i * m_width + j];
				value = m_InputImage[i * m_width + j];
				for (k = -(dlg.m_UpSampleRate-1); k < dlg.m_UpSampleRate; k++){
					for (m = -(dlg.m_UpSampleRate-1); m < dlg.m_UpSampleRate; m++) {
						Re_h = h + (k * m_Re_width);
						Re_w = w + (m);
						if(Re_h >= 0 && (Re_h/m_Re_width) < m_Re_height ){
							if ((Re_w >= 0) && (Re_w < m_Re_width)) {
								if ((Re_h != h) || (Re_w != w)) {
									dis = p_p_x[abs(k)][abs(m)];
									dis2 = dis / (rate * rate);
									temp = value * dis2;
									m_OutputImage[Re_h + Re_w] += int(temp);
								}
							}
						}
					}
...
void CImageProcessingDoc::OnGammaCorrection()
{
	...
	if (dlg.DoModal() == IDOK) {
		for (i = 0; i < m_size; i++) {
			temp = pow(m_InputImage[i], 1 / dlg.m_GammaTrans);
			// 감마 값 계산
			if (temp < 0)
				m_OutputImage[i] = 0;
			else if (temp > 255)
...
void CImageProcessingDoc::OnFft2d()
{
	...
	for (i = 0; i < m_height; i++) {
		for (j = 0; j < m_width; j++) {
			m_OutputImage[i * m_width + j] = temp[i][j]; //오류
		}
	}
